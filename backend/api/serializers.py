...
class RecipeWriteSerializer(serializers.ModelSerializer):
    """Сериализатор для модели Ingredients, работа с данными"""
    ...
    def validate_tags(self, tags):
        if not tags:
            raise ValidationError('В рецепте должен быть тег!')
        if len(tags) != len(set(tags)):
            raise ValidationError(
                'В рецепте не может быть одинаковых тегов!'
            )
        return tags
...
class RecipeMiniSerializer(serializers.ModelSerializer):
    """Сериализатор для модели User, рецепт"""
    class Meta:
        model = Recipe
...
class SubscriptionsSerializerFoodgram(FoodgramUserSerializer):
    """Сериализатор для модели User, рецептовы"""
